---
title: "Application_real_data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Application_real_data}
...
For this application, it used the combined NHANES dataset from 2011-2016 as described in this package, which can be accessed by `data(nhanes.dat)`. The PCB measurements data are extracted. As almost 2/3 of the observations have unit nonresponse, we removed those unit nonresponse and artificially created some missing values that depend on a fully observed variable (PCB138). The total sample size is 6056 with 24 variables.
...
The table above shows the number and percentage of missing, censored and observed values across each variable in the selected PCB dataset.
...
The plot above graphically shows the percentages of missing, censoring and observed values across each variable.
...
The program takes `r round(dur, 2)` minutes to run.
...
The diagnostic plots show that the algorithm works well.
...
We compared the marginal densities of the original available data from each variable with the imputed data from the last iteration of running multiple imputation for both the data on log scale and the original scale.
