...
analysis.data <- as.matrix(select(nhanes.dat$data, starts_with("LBC") & !ends_with("LA")))                      
censoring.ind <- as.matrix(select(nhanes.dat$data, starts_with("LBD")))
LLODs <- as.matrix(nhanes.dat$PCB.LOD)   # lower limits of detection (LLODs)
year <- nhanes.dat$data$SDDSRVYR         # survey year indicator

# rename columns to repsective PCB names
...
We compare the marginal distribution of the data between the imputed dataset from the last iteration and the original dataset.
