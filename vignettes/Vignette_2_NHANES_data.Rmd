...
MLODs <- as.matrix(nhanes.dat$PCB.LOD)   # maximum limits of detection (LMODs)
...
unit.nonresponse <- numeric()
count <- 0
for (i in 1:nrow(analysis.data)) {
  if (sum(is.na(analysis.data[i, ])) == ncol(analysis.data)) {
    count <- count + 1
    unit.nonresponse[count] <- i
  }
}

year <- year[-unit.nonresponse]
analysis.data <- analysis.data[-unit.nonresponse, 1:10]
censoring.ind <- censoring.ind[-unit.nonresponse, 1:10]
```
...
From the tables above, we see that the data has `r analysis.dim[1]` subjects with `r analysis.dim[2]` variables, almost every variable has a few missing values.
...
Next, we have to create a list object for including the censoring bounds information for each of the data point. Specifically, we take missing data as a special type of "censoring" data with two censoring limits corresponding to $-\infty$ and $\infty$, respectively. In this package, we take $-10,000$ and $10,000$ as proxies for $-\infty$ and $\infty$, respectively. We the list as follow,
...
* For the censored values, the corresponding entries in the first matrix are $-10,000$, the corresponding entries in the second matrix are the corresponding maximum limits of detection (MLODs). Note that the measurements fall below MLODs can be viewed as left censored values with the MLODs as the upper censoring limits.
...
for (i in 1:nrow(bounds1)) {
  for (j in 1:ncol(bounds1)) {
    if (censoring.ind[i, j] == 1) {
      ...
      bounds1[i, j] <- -10000
      bounds2[i, j] <- log(analysis.data[i, j] * sqrt(2)) # censored values are MLODs/sqrt(2)
    }
  }
}
...
The tables show the tow matrices, we note that the PCB028 and PCB066 values for the six row are censored since only the lower limit is negative infinity, as represented by $-10,000$. In creating the list, we log-transformed the data to better approach normality, which is the assumption that our package base on, and since the laboratory measurements data can only take positive values. We will transform the imputed dataset back to the original scale after imputation.
...
table(censoring.ind[, "PCB028"])
```

The plots indicate that the imputed data align well with the original observed data, for variables with many censored values, such as PCB28, the peak which indicates a concentration of MLODs, is expected to be shifted a little bit to the left, since we are imputing values smaller than the MLODs.
