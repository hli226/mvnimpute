...
From the table above, we see that the data has `r analysis.dim[1]` subjects with `r analysis.dim[2]` variables, almost every variable has a few missing values.
...
The plots indicate that the imputed data align well with the original data, for variables with many censored values, such as PCB28, the peak which indicates a concentration of MLODs, is expected to be shifted a little bit to the left, since we are imputing values smaller than the MLODs.
