---
title: "mvnimpute_tutorial"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{mvnimpute_tutorial}
...
`miss.indx` and `miss.pos` includes the missing index and positions of missing variables

```{r}
# missing indicators
head(miss.indx)

# positons of missing variables
miss.pos 
```
...
First of all, as the incomplete data includes unobserved data, we need to fill up those values by some simple methods before running multiple imputation. Currently, the **initial.impute** function only incorporates the stochastic regression method in which the unobserved values are generated from the normal distribution with means and variances as the complete-cases means and variances.
...
In the case when the censored values are interval censored. The `censor.val` should be a *list* of length two, of which the first element includes the lower limits, the second element includes the upper limits. If the censoring is either left or right censoring. `censor.val` will be a matrix containing the censoring times instead. `censor.type` specifies the typing of censoring, it takes "right" for right censoring, "left" for left censoring, and "interval" for interval censoring. "interval" is default.
